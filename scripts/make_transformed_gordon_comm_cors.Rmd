...
comm_cors <- comm_cors %>% 
  mutate(study = case_when(
    startsWith(subject_id, "1148") ~ "MINA",
    startsWith(subject_id, "N") ~ "K_study",
    startsWith(subject_id, "T") ~ "K_study"
  ))
comm_cors$study <- ifelse(is.na(comm_cors$study), "TDS", comm_cors$study)
comm_cors$study <- factor(comm_cors$study)

# create grouped (by study) and ungrouped z-scores
comm_cors_z <- comm_cors %>% 
  gather(key = "from_to", value = "cor", -c(subject_id, study)) %>% 
  group_by(study) %>%
...
gridExtra::grid.arrange(
  grobs = setNames(names(comm_cors_z)[c(4,6,5)], names(comm_cors_z)[c(4,6,5)]) %>% 
    purrr::map(~ ggplot(comm_cors_z %>% filter(stringr::str_detect(from_to, "^Auditory")) %>%
                          mutate(group = paste(study, from_to, sep = "_")),
...
comm_cors_wide <- comm_cors_z %>% 
  select(-c("study", "z_score_group", "z_score_ungrouped")) %>% 
  pivot_wider(id_cols = subject_id, names_from = from_to, values_from = cor)

write.csv(comm_cors_wide, "/Users/theresacheng/projects/doa_classification/data/comm_cors.csv", row.names = FALSE) 

comm_cors_z_wide <- comm_cors_z %>% 
  select(-c("study", "cor", "z_score_ungrouped")) %>% 
  pivot_wider(id_cols = subject_id, names_from = from_to, values_from = z_score_group)

write.csv(comm_cors_z_wide[1:length(comm_cors_wide)], "/Users/theresacheng/projects/doa_classification/data/comm_cors_z.csv", row.names = FALSE)
```
